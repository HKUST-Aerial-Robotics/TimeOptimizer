...
void Bernstein::setOrder( double min_order )
{
	...
	{	
		...
		int _min_order_l = floor(_min_order);
		int _min_order_u = ceil (_min_order);

		/*cout<<"min order low"<<_min_order_l<<", min order high"<<_min_order_u<<" min order "<<_min_order<<endl;
		cout<<"low weighting: "<<_min_order_u - _min_order<<", high weighting: "<<_min_order   - _min_order_l<<endl;*/

		if( poly_num1D > _min_order_l )
		{
			for( int i = _min_order_l; i < poly_num1D; i ++ )
			{
		    	for(int j = _min_order_l; j < poly_num1D; j ++ )
		    	{
		      		int coeff = 1.0;
		            int _d = _min_order_l - 1;
		            ...
		            Q_l(i,j) = double(coeff) / double(i + j - 2 * _min_order_l + 1);
		    	}
		  	}
		}

		if( poly_num1D > _min_order_u )
		{
			for( int i = _min_order_u; i < poly_num1D; i ++ )
			{
		    	for(int j = _min_order_u; j < poly_num1D; j ++ )
		    	{
		      		int coeff = 1.0;
		            int _d = _min_order_u - 1;
		            ...
		            Q_u(i,j) = double(coeff) / double(i + j - 2 * _min_order_u + 1);
		    	}
		  	}
		}
		...
		{
			Q_l = (_min_order_u - _min_order)   * Q_l;
			Q_u = (_min_order   - _min_order_l) * Q_u;
		}
